...
async def run_async(args: argparse.Namespace) -> None:
    ...
    async def process_one(ex, epoch_idx):
        ...
        try:
            ...
            return
        # Identify failed agents from this query (Algorithm 1 line 8).
        per_query_failed_agents: set[str] = set()
        for t in gr.trajectories:
            if t.f1 == 0 and t.failed_agent:
                per_query_failed_agents.add(t.failed_agent)
        ...
        # Paper Algorithm 1 lines 8-11: per-query RoPE trigger on identified failed agents
        # when their failure buffer has accumulated enough examples for contrastive analysis.
        if args.rope_per_query:
            for agent_name in per_query_failed_agents:
                if buffer.size(agent_name) < args.rope_min_buffer or agent_name == "Retriever":
                    continue
                try:
...
def main() -> None:
    ...
    ap.add_argument("--rope-update-every", type=int, default=20,
                    help="Periodic batched RoPE round (legacy mode). Unused when --rope-per-query is set.")
    ap.add_argument("--rope-per-query", action="store_true", default=True,
                    help="Paper Algorithm 1: trigger RoPE per query for each identified failed agent.")
    ap.add_argument("--rope-min-buffer", type=int, default=2,
                    help="Min failure-buffer size before per-query RoPE fires for an agent.")
